...
data class State(
    ...
    fun isFinal(): Boolean {
        return rooms[0][0] is Amber && rooms[0][1] is Amber &&
                rooms[1][0] is Bronze && rooms[1][1] is Bronze &&
                rooms[2][0] is Copper && rooms[2][1] is Copper &&
                rooms[3][0] is Desert && rooms[3][1] is Desert
    }
}
...
fun isRoomFinal(state: State, roomIdx: Int): Boolean {
    ...
    if (room[0] == null && room[1] != null) {
        return room[1]!!.room == roomIdx
    } else if (room[0] != null && room[1] != null) {
        return room[0]!!.room == roomIdx && room[1]!!.room == roomIdx
    } else {
        return false
    }
}
...
fun tryHallway(state: State, hallwayIdx: Int): Long? {
    ...
    if (room[0] == null && room[1] != null && room[1]!!.javaClass == amphipod.javaClass) {
        ...
        // can move to the bottom of the room
        ...
        throw IllegalStateException("this shouldn't happen: ${room}")
    }
    return best
}
...
fun main() {
    val input =
        loadFromResources("day23.txt").readLines().drop(2).take(2).map { it.replace("#", "").replace(" ", "") }.map {
            it.map(::parse)
        }
    val initialState = State(
        List(11) { null }, listOf(
            listOf(input[0][0], input[1][0]),
            listOf(input[0][1], input[1][1]), listOf(input[0][2], input[1][2]), listOf(input[0][3], input[1][3])
        )
    )
